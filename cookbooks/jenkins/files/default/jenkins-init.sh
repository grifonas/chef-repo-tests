...
# Allow passing parameters between jobs:
ALLOW_PARAMETERS="-Dhudson.model.ParametersAction.keepUndefinedParameters=true"
...
do_start()
{
    ...
    $SU -l $JENKINS_USER --shell=/bin/bash -c "$DAEMON $DAEMON_ARGS -- $JAVA $JAVA_ARGS $ALLOW_PARAMETERS -jar $JENKINS_WAR $JENKINS_ARGS" || return 2
}
